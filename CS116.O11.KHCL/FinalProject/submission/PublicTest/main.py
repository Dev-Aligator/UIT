...
def run_train(train_dir, dev_dir, model_dir):
    ...
    # X_train_slope = X_train.drop(['RoadSlope_100ms'],axis = 1)

    #X_train = train_data.drop(columns=['Vehicle_Mass','RoadSlope_100ms'])
    # X_train_mass = X_train.drop(['Vehicle_Mass','RoadSlope_100ms'],axis = 1)
    # X_train_slope = X_train_mass
    # X_train_slope = scaler_slope.fit_transform(X_train_slope)
    # X_train_mass = scaler_mass.fit_transform(X_train_mass)
    #X_train_mass = transformer.transform(X_train_mass)
    #X_train_slope = transformer.transform(X_train_slope)
    ...
    model_slope = KNeighborsRegressor(leaf_size=50, n_neighbors=1, p=1, metric='manhattan', weights='distance')
    ...
    # scaler_slope_path = os.path.join(model_dir, 'scaler_slope.joblib')
    # dump(scaler_mass, scaler_mass_path)
...
def run_predict(model_dir, input_dir, output_path):
    ...
    # X_test_mass = scaler_mass.transform(X_test)
